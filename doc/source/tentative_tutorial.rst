...
creating a root-level logger and adding a handler to it. All information in
...
With the OptionsManager you also determine the type of solver you want to use
for linear optimization (if any).
...
Let's say we have an SBML document containing a metabolic model of interest.
When parsing an SBML document a MetabolicSystem instance is returned.

>>> system = pymetabolism.parse("path/to/model.xml")
...
The bipartite network relies on the networkx package and its DiGraph class.
So far few methods of the networkx.DiGraph class are overridden. If you do evil
operations the bipartite structure may be destroyed and many networkx algorithms
may yield strange results due to the existing bipartite nature. So be careful.
